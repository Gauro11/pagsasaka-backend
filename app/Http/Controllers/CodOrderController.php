...
class CODOrderController extends Controller
{
    ...
    public function createCODOrder(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->items as $item) {
                ...
                Log::info("Order ID {$order->id} with Order Number {$orderNumber} created for Product ID {$product->id} by Account ID {$userId}");
            ...
            foreach ($orders as $item) {
                $emailBody .= "Order #{$item['order_number']} - {$item['quantity']}x {$item['product']} - ₱" . number_format($item['total_amount'], 2) . "\n";
            }
    ...
    // Add this method at the bottom of your controller
