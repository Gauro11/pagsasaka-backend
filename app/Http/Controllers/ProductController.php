    ...
    public function getAllProductsList(Request $request)
{
    try {
        ...
        $query = Product::select(
            ...
            ->leftJoin('orders', 'products.id', '=', 'orders.product_id') // Join to fetch ship_to
            ->where('products.is_archived', '0') // Assuming we only want active products
            ->when($searchTerm, function ($query, $searchTerm) {
                return $query->where(function ($activeQuery) use ($searchTerm) {
                    $activeQuery->where('products.product_name', 'like', '%' . $searchTerm . '%')
        ...
        $formattedProducts = $result->getCollection()->transform(function ($product) {
            ...
            return [
                ...
                'ship_to' => $product->ship_to, // Assign ship_to fetched from the orders table
            ];
        });
